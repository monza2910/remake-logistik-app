...
class Logisticton extends Component
{
    public $diskon = '0';
    public $name , $weight;
    ...
    public function render()
    {
        ...
        if (!empty($this->service) &&!empty($this->to) && !empty($this->from)) {
            ...
            foreach ($prices as $price ) {
                $one_ton = $price['one_ton'];
                $five_ton = $price['five_ton'];
                $tenton = $price['ten_ton'];
            }

            if ($this->sub_berat >= 1 && $this->sub_berat <= 5) {
                $this->harga_kg = $one_ton;
            } elseif($this->sub_berat > 5 && $this->sub_berat <10) {
                $this->harga_kg = $five_ton;
            } elseif($this->sub_berat >= 10 ) {
                $this->harga_kg = $tenton;
            }
        ...
        $items = \Cart::session('logisticton')->getContent()->sortBy(function($cart){
        ...
        $this->sub_berat =\Cart::session('logisticton')->getTotalQuantity();
        ...
        return view('livewire.logisticton',
        [
         'carts' => $cartData,
         'origins'  => $origins,
    ...
    public function addItem(){
        ...
        $rowId = "Cart".$id;
        $cart = \Cart::session('logisticton')->getContent();
        $cekItemId = $cart->whereIn('id', $rowId);
        if($cekItemId->isNotEmpty()){
            \Cart::session('logisticton')->update($rowId,[
                'quantity' =>[
                    'relative' =>true,
                    'value'  => $this->weight,
                ]
    ...
    public function minItem($rowId){
        ...
        if($checkItemId[$rowId]->quantity > 1){
            ...
            \Cart::session('logisticton')->remove($rowId);
            $this->resetFieldsService();
    ...
    public function submitHandle(){
        ...
        try {
            $allcart = \Cart::session('logisticton')->getContent();
            ...
            if (\Cart::isEmpty()) {
                ...
                \Cart::session('logisticton')->clear();
                $this->resetFieldsAll();
